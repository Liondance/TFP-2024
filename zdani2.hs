...
-- testing pending
data M =
      Show E            -- magic form: .show(<exp>) prints <string> ==> rvalue(<exp>)
...
-- | A way of making a parser a lexeme
lexeme :: Monad m => Parser m a -> Parser m a
lexeme p = p <* spaces

-- | A way of making a string a lexeme
...
-- | A shorthand for running a parser
...
pProgram :: Monad m => Parser m Prog
...
pFile :: String -> [Either ParseError Prog]
...
-- | Use this for '.show' magic command. First argument is an evaluator.
showE :: Show a => (E -> a) -> E -> String
showE f e =  ".show" <> mkParens True (prettyExp e) <> "===>" <> show (f e)
...
testMagic = runTestTT . TestList $ 
  [ "Parsing: .show(f(g(minus(5)(y))(f))) should be: Magic . Show $ Apply (Sym \"f\") (Apply (Apply (Sym \"g\") (Minus (Val 5) (Sym \"y\"))) (Sym \"f\"))"
    ~: Right (Magic . Show $ Apply (Sym "f") (Apply (Apply (Sym "g") (Minus (Val 5) (Sym "y"))) (Sym "f"))) ~=? parse' (fully pAction) ".show(f(g(minus(5)(y))(f)))"
  , "Parsing: .env should be: Magic ShowEnv"
    ~: Right (Magic ShowEnv) ~=? parse' (fully pAction) ".env"
  ]
